...
class ClsSolver(BaseSolver):
    ...
    def build_model(self):
        ...
        ######################################################
        self.model_teacher = models.resnet50(pretrained=False)
        pthfile = '/mnt/lustre/xingxingrun/prototype38/resnet50-19c8e357.pth'
        self.model_teacher.load_state_dict(torch.load(pthfile))
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_data['loader']):
            ...
            # forward
            logits, logits_dist = self.model(input)  ############################
            # mixup
            ...
            ############################################
            logits_teacher = self.model_teacher(input)
            loss = (loss + self.KD_loss(logits_dist, logits_teacher.detach()) / self.dist.world_size) / 2.0
            ############################################
